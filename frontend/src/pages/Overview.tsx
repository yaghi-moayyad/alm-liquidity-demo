 ...
 return <>
  ...
  {!result?<Empty title="No completed cash-flow run yet" description="Run a calculation to populate the liquidity position. Regulatory reports remain available from navigation." action={<Button variant="contained" onClick={()=>nav('/new')}>Start calculation</Button>}/>:<>
   ...
   {!profile?<Alert severity="info" sx={{mb:3}}>This run has no compatible bank-format ladder for {currency}. Create a new run to enable reconciled liquidity analytics.</Alert>:<>
    ...
    <Box sx={{display:'grid',gridTemplateColumns:{xs:'1fr',lg:'minmax(0,1.35fr) minmax(320px,1fr)'},gap:2.5,mb:2.5}}>
     <Card><SectionHead title="Regulatory trend" subtitle="LCR and NSFR over the loaded month-end snapshots · open a report to explore any month" action={<Stack direction="row" gap={1}><Button size="small" onClick={()=>nav('/lcr')}>LCR</Button><Button size="small" onClick={()=>nav('/nsfr')}>NSFR</Button></Stack>}/>
      <Box sx={{height:300,px:1.2,pb:1}}><ResponsiveContainer width="100%" height="100%"><LineChart data={regulatoryTrend} margin={{top:10,right:18,bottom:8,left:0}}><CartesianGrid vertical={false} stroke="#E8EEF3" strokeDasharray="3 4"/><XAxis dataKey="label" tick={{fontSize:10}} interval="preserveStartEnd"/><YAxis tickFormatter={(value:number)=>`${value.toFixed(0)}%`} width={55} tick={{fontSize:10}}/><Tooltip formatter={(value)=>`${Number(value).toFixed(1)}%`}/><Legend wrapperStyle={{fontSize:11}}/><Line type="monotone" connectNulls dataKey="lcr" name="LCR" stroke="#3869C9" strokeWidth={3} dot={{r:3}}/><Line type="monotone" connectNulls dataKey="nsfr" name="NSFR" stroke="#248A77" strokeWidth={3} dot={{r:3}}/></LineChart></ResponsiveContainer></Box>
      <Divider/><Typography variant="caption" color="text.secondary" display="block" px={2.5} py={1.5}>Ratios are calculated from the saved regulatory source snapshots. Open LCR or NSFR to select any point and see its exact explanation.</Typography>
     </Card>
     <Card><SectionHead title="Near-term obligations and capacity" subtitle="Scheduled amounts by maturity bucket" action={<Button size="small" onClick={()=>analytics()} endIcon={<ArrowForwardRounded/>}>Explore</Button>}/>
      <Box sx={{height:300,px:1,pb:1}}><ResponsiveContainer width="100%" height="100%"><BarChart data={profile.buckets} onClick={state=>{const item=(state as {activePayload?:{payload?:{code:string}}[]})?.activePayload?.[0]?.payload;if(item)analytics(item.code)}} margin={{top:10,right:10,bottom:8,left:0}}><CartesianGrid vertical={false} stroke="#E8EEF3" strokeDasharray="3 4"/><XAxis dataKey="label" tick={{fontSize:10}} interval="preserveStartEnd"/><YAxis tickFormatter={(value:number)=>compact(value)} width={60} tick={{fontSize:10}}/><Tooltip formatter={(value)=>money(Number(value),currency)}/><Legend wrapperStyle={{fontSize:10}}/><Bar dataKey="outflows" name="Outflows" fill="#6688C9" maxBarSize={18}/><Bar dataKey="offBalance" name="Off-balance sheet" fill="#C18B50" maxBarSize={18}/><Bar dataKey="capacity" name="Capacity" fill="#54ACC4" maxBarSize={18}/></BarChart></ResponsiveContainer></Box>
      <Divider/><Typography variant="caption" color="text.secondary" display="block" px={2.5} py={1.5}>A management view from the bank-format ladder; capacity eligibility and encumbrance need separate validation.</Typography>
     </Card>
    </Box>
