...
export type LadderRow={section:string;key:string;label:string;kind:'normal'|'subtotal'|'gap'|'cumulative';balance:string;principal:string[];interest:string[];total:string[]};
export type BankLadder={buckets:{code:string;label:string}[];rows:LadderRow[]};
export type Result=Validation&{engine_version:string;as_of_date:string;entity:string;bucket_days:number[];input_count:number;currencies:string[];summary:Record<string,Bucket[]>;bank_ladder:Record<string,BankLadder>;interest_projection:'constant'|'forward_curve';basis:string;status:string};
